...
#pragma unroll
        ...
        {
            sum += c_Kernel[kernel_radius - j] * s_Data[threadIdx.z][threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
        }
...
#pragma unroll
    ...
    for (int i = COLUMNS_HALO_STEPS; i < COLUMNS_HALO_STEPS + COLUMNS_RESULT_STEPS; i++)
    {
        ...
        s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y][threadIdx.z]= (global_y < imageH) ? d_Src[i * COLUMNS_BLOCKDIM_Y * pitch] : 0;
...
#pragma unroll
    ...
    {
        s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y][threadIdx.z] = (baseY >= -i * COLUMNS_BLOCKDIM_Y) ? d_Src[i * COLUMNS_BLOCKDIM_Y * pitch] : 0;
    }
...
#pragma unroll
    ...
    {
        s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y][threadIdx.z] = (imageH - baseY > i * COLUMNS_BLOCKDIM_Y) ? d_Src[i * COLUMNS_BLOCKDIM_Y * pitch] : 0;
...
#pragma unroll
        ...
        for (int j = -kernel_radius; j <= kernel_radius; j++)
        {
            sum += c_Kernel[kernel_radius - j] * s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y + j][threadIdx.z];
        }
...
////////////////////////////////////////////////////////////////////////////////
// Slices convolution filter
////////////////////////////////////////////////////////////////////////////////
#define   SLICES_BLOCKDIM_X 4
#define   SLICES_BLOCKDIM_Y 4
#define   SLICES_BLOCKDIM_Z 16
#define   SLICES_RESULT_STEPS 4
#define   SLICES_HALO_STEPS 1

extern "C" __global__ void convolutionSlicesKernel(
    float *d_Dst,
    const float *d_Src,
    int imageW,
    int imageH,

...
#include "cuda_operation_convolution.h"
...
CudaOperationConvolution3D::CudaOperationConvolution3D()
  : CudaOperationBase("CUDA Convolution 3D")
{
}

bool CudaOperationConvolution3D::Initialize(const OperationParameters* params)
{
  initialized_ = false;
  ...
  std::strcat(exec_path, "/kernels/convolution_3d.ptx");
  ...
  if (!CheckCudaError(cuModuleLoad(&cu_module_, exec_path))) {
    if (!CheckCudaError(cuModuleGetFunction(&cuf_convolution_rows_, cu_module_, "convolutionRowsKernel")) &&
        ...
        !CheckCudaError(cuModuleGetFunction(&cuf_convolution_slices_, cu_module_, "convolutionSlicesKernel"))
        ) {
      size_t const_size;
...
void CudaOperationConvolution3D::ComputeGaussianKernel(float sigma, size_t precision, float pixel_size)
{
...
void CudaOperationConvolution3D::PrintConvolutionKernel()
{
    if (kernel_ == nullptr)
...
void CudaOperationConvolution3D::Execute(OperationParameters& params)
{  
  if (!IsInitialized()) {
    return;
  }
...
void CudaOperationConvolution3D::ResampleRows(CUdeviceptr input, CUdeviceptr output, DataSize4& data_size, size_t pitch) const
{
    ...
    dim3 row_grid_dim ={ static_cast<unsigned int>((data_size.width  + (row_block_dim.x * rows_results_steps) - 1) / (row_block_dim.x * rows_results_steps)),
        ...
        static_cast<unsigned int>((data_size.depth + row_block_dim.z - 1) / row_block_dim.z) };
    ...
    int needed_shared_memory_size =
        ((rows_results_steps + 2*rows_halo_steps)*row_block_dim.x) * (row_block_dim.y) * (row_block_dim.z) * sizeof(float);
...
void CudaOperationConvolution3D::ResampleColumns(CUdeviceptr input, CUdeviceptr output, DataSize4& data_size, size_t pitch) const
{
    int kernel_radius = static_cast<int>(kernel_radius_);
    ...
    dim3 col_grid_dim ={ static_cast<unsigned int>((data_size.width  + col_block_dim.x - 1) / col_block_dim.x),
        ...
        static_cast<unsigned int>((data_size.depth  + col_block_dim.z - 1) / col_block_dim.z) };
    ...
    int needed_shared_memory_size =
        (col_block_dim.x * ((cols_results_steps + 2*cols_halo_steps)*col_block_dim.y + 0)) * col_block_dim.z * sizeof(float);
